...
export default function AlertPage () 
{
	...
	const [alert, setAlerts] = useState<any[]>([])
	...
	const load = async () => {
		...
		useEffect(() => {load() },[])
		 ...
		 const handleResolve = async(id: string ) => {
			 ...
			 const { error } = await supabase.from('alert').update({resolved: true }).eq('id',id)
		     ...
		     setAlerts(alert.map(a => a.id=== id ? { ...a, resolved:true } : a))
			...
			{alerts.length ===0 ? (
			   ...
			   <p className="bg-yellow-50 p-4 border rounded">No alerts triggered yet.</p>
				 ...
				 {Alerts.map(a => (
								   ...
								   <div>
								     <p className="font-medium">{sites[a.sites_id] ||'Unknown sites'}</p>
